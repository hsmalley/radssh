...
def _importKey(filename, allow_prompt=True, logger=None):
    ...
    try:
        ...
        while retries:
            try:
                ...
                if logger:
                    logger.debug(
                        "Loaded passphrase protected RSA key from %s", filename
                    )
                return key
    ...
    try:
        ...
        while retries:
            try:
                ...
                if logger:
                    logger.debug(
                        "Loaded passphrase protected ECDSA key from %s", filename
                    )
                return key
            except paramiko.SSHException as e:
                print(repr(e))
    ...
    try:
        ...
        while retries:
            try:
                passphrase = user_password(
                    "Enter passphrase for Ed25519 key [%s]: " % filename
                )
                key = paramiko.Ed25519Key(filename=filename, password=passphrase)
                if logger:
                    logger.debug(
                        "Loaded passphrase protected Ed25519 key from %s", filename
                    )
                return key
            except paramiko.SSHException as e:
                print(repr(e))
    ...
    if logger:
        logger.debug(
            "Failed to load %s as Ed25519 key\n\t%s", filename, repr(ecdsa_exception)
        )
    ...
    try:
        ...
        while retries:
            try:
                ...
                if logger:
                    logger.debug(
                        "Loaded passphrase protected DSA key from %s", filename
                    )
                return key
            except paramiko.SSHException as e:
                print(repr(e))
...
class AuthManager(object):
            ...
            with open(auth_file, "r") as f:
                for line_no, line in enumerate(f, 1):
                    ...
                    if fields[0] == "password" or len(fields) == 1:
                        ...
                        self.logger.info(
                            "PlainText password loaded from %s (line %d)",
                            auth_file,
                            line_no,
                        )
                        ...
                        try:
                            ...
                            if encrypted_password.decoder_ring.unsupported:
                                ...
                                self.logger.error(
                                    "PKCS encryption not supported by cryptography module - Ignoring encrypted password from %s (line %d)",
                                    auth_file,
                                    line_no,
                                )
                                continue
                            ...
                            self.logger.info(
                                "PKCS encrypted password loaded from %s (line %d)",
                                auth_file,
                                line_no,
                            )
                            ...
                            warnings.warn(
                                RuntimeWarning(
                                    "Failed to load base64 PKCS encrypted password from %s (line %d)\n\t%s"
                                    % (auth_file, line_no, repr(e))
                                )
                            )
                            self.logger.error(
                                "Failed to load base64 PKCS encrypted password from %s (line %d)\n\t%s",
                        ...
                        if os.path.exists(k):
                            ...
                            self.logger.info(
                                "Deferred load of SSH private key [%s] from %s (line %d)",
                                k,
                                auth_file,
                                line_no,
                            )
                            ...
                            self.logger.error(
                                "Nonexistent private key file [%s] referenced by %s (line %d)",
                                k,
                                auth_file,
                                line_no,
                            )
                        ...
                        warnings.warn(
                            RuntimeWarning(
                                "Unsupported auth type [%s:%d] %s"
                                % (auth_file, line_no, fields[0])
                            )
                        )
                        self.logger.error(
                            'Unsupported auth type "%s" referenced in %s (line %d)',
                            fields[0],
                            auth_file,
                            line_no,
                        )
    ...
    def authenticate(self, T, sshconfig={}):
        ...
        for auth_type in sshconfig.get(
        ...
        for auth_type in preferred_auth_types:
            ...
            if (
                auth_type == "publickey"
                and sshconfig.get("pubkeyauthentication", "yes") == "yes"
            ):
                # Try explicit keys first
                identity_keys = []
                for keyfile in sshconfig.get(
                    "identityfile",
                    [
                        "~/.ssh/id_rsa",
                        "~/.ssh/id_ed25519",
                ...
                auth_success = self.try_auth(
                    T, identity_keys, False, auth_user, allow_prompt=allow_prompt
                )
                if auth_success:
                    break
                if sshconfig.get("identitiesonly", "no") == "no":
                    ...
                    auth_success = self.try_auth(
                        T, self.keys, False, auth_user, allow_prompt=allow_prompt
                    )
                    if auth_success:
                        break
                ...
                auth_type == "password"
                and sshconfig.get("passwordauthentication", "yes") == "yes"
            ) or (
                auth_type == "keyboard-interactive"
                and sshconfig.get("kbdinteractiveauthentication", "yes") == "yes"
            ):
                # Paramiko will fake keyboard-interactive as password authentication
                auth_success = self.try_auth(
                    T, self.passwords, True, auth_user, allow_prompt=allow_prompt
                )
                if auth_success:
                    break
                ...
                if self.default_passwords.get(None):
                    auth_success = self.try_auth(
                        T, [(None, self.default_passwords[None])], True, auth_user
                    )
                ...
                with self.import_lock:
                    ...
                    while not auth_success and retries > 0 and T.is_active():
                        if not password:
                            password = PlainText(
                                user_password(
                                    "Please enter a password for (%s@%s) :"
                                    % (auth_user, T.getName())
                                )
                            )
                            retries -= 1
                        auth_success = self.try_auth(
                            T, [(None, password)], True, auth_user
                        )
                        if auth_success:
    ...
    def interactive_password(self):
        self.default_passwords[None] = PlainText(
            user_password("Please enter a password for (%s) :" % self.default_user)
        )
    ...
    def try_auth(
        self, T, candidates, as_password=False, auth_user=None, allow_prompt=True
    ):
        if not auth_user:
            ...
            try:
                if as_password:
                    try:
                        ...
                        self.logger.debug(
                            "Unusable password value (%s): [%s]", str(e), repr(value)
                        )
                        continue

                    self.logger.debug(
                        "Trying password (%s) for %s",
                        "*" * len(key),
                        T.getpeername()[0],
                    )
                    ...
                    self.logger.debug(
                        "Trying private key (%s) for %s",
                        repr(value),
                        T.getpeername()[0],
                    )
                    if (
                        isinstance(value, paramiko.AgentKey)
                        ...
                        with self.import_lock:
                            if not self.deferred_keys[value]:
                                self.deferred_keys[value] = _importKey(
                                    value, allow_prompt=allow_prompt, logger=self.logger
                                )
                    ...
                    try:
                        if isinstance(key, paramiko.PKey):
                            ...
                            self.logger.error(
                                "Skipping SSH key %s (%s)", value, str(key)
                            )
...
if __name__ == "__main__":
    ...
    for authfile in sys.argv[1:]:
        ...
        sample = AuthManager(
            os.path.basename(authfile), auth_file=authfile, default_password=""
        )
        ...
        if sample.default_passwords:
            print(
                "Authfile includes a default password [%s]"
                % repr(sample.default_passwords)
            )
