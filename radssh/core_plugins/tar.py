...
tar_options = {"*tar": "-cv", "*tgz": "-cvz", "*tbz": "-cvj"}
    ...
    for host, job in res.items():
        ...
        if job.completed and result.return_code == 0:
            outfile = os.path.join(
                logdir, "tarfile_%d_%s.%s" % (tar_number, str(host), cmd.split()[0][1:])
            )
...
star_commands = {"*tar": tar_command, "*tgz": tar_command, "*tbz": tar_command}
