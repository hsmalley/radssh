...
def init(**kwargs):
    ...
    if os.path.exists(os.path.expanduser("~/.bash_profile")):
        cmd = ["bash", "-ic", "source ~/.bash_profile; alias| sed -e 's/^alias //'"]
    elif os.path.exists(os.path.expanduser("~/.bashrc")):
        cmd = ["bash", "-ic", "source ~/.bashrc; alias| sed -e 's/^alias //'"]
...
star_commands = {"*alias": print_aliases, "*history": star_history}
