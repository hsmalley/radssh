...
def run_local_command(original_name, remote_hostname, port, remote_username, sshconfig):
        ...
        translations = {
            ...
            "%C": hashlib.sha1(
                (
                    ...
                    + remote_hostname
                    + str(port)
                    + sshconfig.get("user", remote_username)
                ).encode("UTF8")
            ).hexdigest(),
        }
    ...
    logging.getLogger("radssh").info(
        'Executing LocalCommand "%s" for connection to %s', cmd, original_name
    )
    # Self-inflicted harm if this never returns...
    p = subprocess.Popen(shlex.split(cmd))
    logging.getLogger("radssh").debug(
        'LocalCommand "%s" completed with return code %d', cmd, p.wait()
    )
...
def connection_worker(host, conn, auth, sshconfig={}):
    ...
    if isinstance(conn, str):
        ...
        if proxy:
            logging.getLogger("radssh").info(
                'Connecting to %s via ProxyCommand "%s"', hostname, proxy
            )
            ...
            try:
                ...
                logging.getLogger("radssh").error(
                    'Invalid ConnectTimeout value "%s" ignored: %s', timeout, e
                )
                timeout = None
        ...
        if ciphers:
            ...
            for name in ciphers.split(","):
                if name in t._preferred_ciphers:
                    ...
                    logging.getLogger("radssh").debug(
                        "Ignoring cipher %s (not supported by Paramiko)", name
                    )
            t._preferred_ciphers = tuple(preferred)
            logging.getLogger("radssh").debug(
                "Setting Paramiko _preferred_ciphers to %s", t._preferred_ciphers
            )
        kex_algorithms = sshconfig.get("kexalgorithms")
        if kex_algorithms:
            logging.getLogger("radssh").debug(
                "Limit KexAlgorithms to %s", kex_algorithms
            )
            preferred = []
            for name in kex_algorithms.split(","):
                if name in t._preferred_kex:
                    ...
                    logging.getLogger("radssh").debug(
                        "Ignoring KexAlgorithm %s (not supported by Paramiko)", name
                    )
            t._preferred_kex = tuple(preferred)
            logging.getLogger("radssh").debug(
                "Setting Paramiko _preferred_kex to %s", t._preferred_kex
            )
        ...
        if macs:
            ...
            for name in macs.split(","):
                if name in t._preferred_macs:
                    ...
                    logging.getLogger("radssh").debug(
                        "Ignoring MAC %s (not supported by Paramiko)", name
                    )
            t._preferred_macs = tuple(preferred)
            logging.getLogger("radssh").debug(
                "Setting Paramiko _preferred_macs to %s", t._preferred_macs
            )
    ...
    if loglevel.upper() in sshconfig_loglevels:
        logging.getLogger(t.get_log_channel()).setLevel(
            sshconfig_loglevels[loglevel.upper()]
        )
    else:
        logging.getLogger("radssh").warning(
            "Unknown LogLevel (%s) for %s", loglevel, host
        )

    try:
        if check_host_key:
            verify_host = sshconfig.get("hostkeyalias", str(hostname))
            sys_known_hosts = known_hosts.load(
                sshconfig.get("globalknownhostsfile", "/etc/ssh/ssh_known_hosts")
            )
            user_known_hosts = known_hosts.load(
                sshconfig.get("userknownhostsfile", "~/.ssh/known_hosts")
            )
            ...
            if keys:
                ...
                t._preferred_keys = sshconfig.get(
        ...
        logging.getLogger("radssh").error(
            "Unable to verify host key for %s\n%s", verify_host, repr(e)
        )
...
def exec_command(host, t, cmd, quota, streamQ, encoding="UTF-8"):
    ...
    if isinstance(t, paramiko.Transport) and t.is_authenticated():
        stdout = StreamBuffer(
            streamQ, (str(host), False), blocksize=2048, encoding=encoding
        )
        stderr = StreamBuffer(
            streamQ, (str(host), True), blocksize=2048, encoding=encoding
        )
        ...
        while not (stdout_eof and stderr_eof and s.exit_status_ready()):
            ...
            if quota.time_exceeded(quiet_time):
                process_completion = (
                    "*** Time Limit (%d) Reached ***" % quota.time_limit
                )
                break
            if quota.bytes_exceeded(len(stdout)):
                process_completion = (
                    "*** Byte Limit (%d) Reached ***" % quota.byte_limit
                )
                break
            if quota.lines_exceeded(stdout.line_count):
                process_completion = (
                    "*** Line Limit (%d) Reached ***" % quota.line_limit
                )
                break
        ...
        if stdout.discards:
            logging.getLogger("radssh").warning(
                "StreamBuffer encountered %d discards", stdout.discards
            )
            process_completion += (
                "StreamBuffer encountered %d discards" % stdout.discards
            )
...
def sftp_thread(host, t, srcfile, dstfile=None, attrs=None):
    ...
    s.close()
    return CommandResult(
        command="SFTP %s -> %s" % (srcfile, dstfile),
        return_code=0,
        status="*** Complete ***",
        stdout="Transferred %d bytes" % attrs.st_size,
        stderr="",
    )
...
class Cluster(object):
    ...
    def __init__(
        ...
        if console:
            ...
            if start_threads:
                ...
                # Lightweight no-thread console used for testing
                class _NoopConsole:
                    def __init__(self, q):
                        self.q = q
        ...
        if defaults.get("ssh_config"):
            logging.getLogger("radssh").warning(
                "Loading SSH Config file: %s", defaults["ssh_config"]
            )
            try:
                ...
                logging.getLogger("radssh").warning(
                    "Unable to process user ssh_config file: %s", e
                )
            ...
            try:
                ...
                logging.getLogger("radssh").warning(
                    "Unable to process system ssh_config file (%s): %s",
                    system_config,
                    e,
                )
        ...
        for label, conn in hostlist:
            ...
            if mux:
                for idx, mux_var in enumerate(mux.get(label, [])):
                    ...
                    self.pending[
                        self.dispatcher.submit(
                            connection_worker, mux_label, conn, self.auth, ssh_cfg
                        )
                    ] = label
                ...
                self.pending[
                    self.dispatcher.submit(
                        connection_worker, label, conn, self.auth, ssh_cfg
                    )
                ] = label
        self.update_connections()
        # Start remainder of dispatcher threads if requested
        if start_threads:
    ...
    def update_connections(self):
        ...
        while True:
            try:
                    ...
                    try:
                        if transport.is_authenticated():
                            transport.set_keepalive(
                                int(self.defaults.get("keepalive", 0))
                            )
                            ...
                            for id_string in self.defaults.get("force_tty", "").split(
                                ...
                                if id_string and id_string in transport.remote_version:
                                    self.console.message(
                                        "%s (%s)" % (host, transport.remote_version),
                                        "FORCE TTY",
                                    )
                        ...
                        logging.getLogger("radssh.connection").warning(
                            "Failed to connect to %s: %s" % (host, str(transport))
                        )
                break
                ...
                self.console.message(
                    "Aborting %d pending connections" % len(self.pending), "Ctrl-C"
                )
                for label in self.pending.values():
                    ...
                    logging.getLogger("radssh.connection").warning(
                        "Aborted connect to %s: Ctrl-C" % host
                    )
                self.pending.clear()
                ...
                new_dispatcher = Dispatcher(
                    outQ=queue.Queue(), threadpool_size=self.dispatcher.threadpool_size
                )
    ...
    def reauth(self, user):
        ...
        if not user or user == self.auth.default_user:
            ...
            retry = AuthManager(
                self.auth.default_user, auth_file=None, try_auth_none=False
            )
        else:
            # Give the option of reusing the existing auth options with new user
            alternate_password = user_password(
                "Please enter a password for (%s) or leave blank to retry auth options with new user:"
                % user
            )
            if alternate_password:
                retry = AuthManager(
                    user, auth_file=None, default_password=alternate_password
                )
            else:
                retry = self.auth
        ...
        for k, t in self.connections.items():
            ...
            try:
                conn = socket.create_connection(
                    (str(k), 22),
                    timeout=float(self.defaults.get("socket.timeout", 2.0)),
                )
            ...
            self.pending[
                self.dispatcher.submit(
                    connection_worker, k, conn, retry, {"identityfile": []}
                )
            ] = k
        self.update_connections()
    ...
    def tunnel_connections(self, hostlist, jumpbox=None):
        ...
        for host in hostlist:
            try:
                ...
                self.console.q.put(
                    (("TUNNEL", True), "Unable to tunnel to %s: %s" % (host, e))
                )
    ...
    def run_command(self, template):
        ...
        for chunk in chunker:
                ...
                if self.output_mode == "stream":
                    self.pending[
                        self.dispatcher.submit(
                            ...
                            self.quota,
                            self.console.q,
                            self.defaults["character_encoding"],
                        )
                    ] = k
                else:
                    self.pending[
            ...
            while self.pending:
                try:
                    self.console.status(
                        "Completed on %d/%d hosts" % (len(result), total)
                    )
                    ...
                    if time.time() - last_interrupt < 2.0:
                        ...
                        in_flight = sorted(
                            [str(k) for k in self.pending.values() if k not in result]
                        )
                        ...
                        self.console.q.put(
                            (
                                ("CONSOLE", True),
                                "In-Flight commands running on %s" % str(in_flight),
                            )
                        )
                        self.console.q.put(
                            (
                                ("CONSOLE", True),
                                "To kill: Press <Ctrl-C> again within 2 seconds",
                            )
                        )
                except Exception as e:
                ...
                if isinstance(v, CommandResult):
                    ...
                    if v.stderr:
                        if self.log_err:
                            ...
                            if lines:
                                with open(
                                    os.path.join(logdir, self.log_err), "ab"
                                ) as f:
                                    f.write(
                                        (
                                            '[%s] === "%s" %s [%s] ===\n'
            ...
            try:
                for pid, summary in self.dispatcher.async_results():
                    ...
                    if not summary.completed:
                        self.console.message(
                            "%s - %s" % (str(host), repr(summary.result)), "EXCEPTION"
                        )
                    self.console.status(
                        "Completed on %d/%d hosts" % (total - len(self.pending), total)
                    )
            except UnfinishedJobs:
                pass
            except KeyboardInterrupt:
            ...
            if isinstance(t, paramiko.Transport):
                if not t.is_active():
                    ...
                    bad.append(
                        (
                            k,
                            "(%7.3fs) Connected to %s / not authenticated"
                            % (connect_time, t.getpeername()[0]),
                        )
                    )
                else:
                    if k in self.disabled:
                        good.append(
                            (
                                k,
                                "(%7.3fs) Authenticated as %s to %s (Disabled)"
                                % (connect_time, t.get_username(), t.getpeername()[0]),
                            )
                        )
                    else:
                        good.append(
    ...
    def close_connections(self):
        ...
        for k in list(self.connections):
            ...
            self.dispatcher.submit(
                close_connection, t, k, self.defaults.get("force_tty.signoff", "")
            )
