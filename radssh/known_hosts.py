    ...
    with _lock:
        if filename not in _loaded_files:
            try:
                ...
                logging.getLogger("radssh.keys").info(
                    "Unable to load known_hosts from %s: %s" % (filename, str(e))
                )
...
def verify_transport_key(t, hostname, port, sshconfig):
    ...
    for x in keys:
        if x.key.get_name() == hostkey.get_name():
            ...
            logging.getLogger("radssh.keys").warning(
                "Host %s failed SSH key validation - conflicting entry [%s:%d]"
                % (hostname, x.filename, x.lineno)
            )
            raise Exception(
                "Host %s failed SSH key validation - conflicting entry [%s:%d]"
                % (hostname, x.filename, x.lineno)
        ...
        if sshconfig.get("stricthostkeychecking", "ask") == "yes":
            logging.getLogger("radssh.keys").warning(
                "No host key found for %s and StrictHostKeyChecking=yes" % hostname
            )
    ...
    if sshconfig.get("checkhostip", "no") == "yes":
        ...
        for x in keys:
            ...
            if sshconfig.get("stricthostkeychecking", "ask") == "yes":
                logging.getLogger("radssh.keys").warning(
                    "No host key found for IP %s (%s) and StrictHostKeyChecking=yes"
                    % (verify_ip, hostname)
                )
                raise Exception(
                    "Missing known_hosts entry for IP: %s (%s)" % (verify_ip, hostname)
                )
            add_ip_entry = True
    ...
    if sshconfig.get("hashknownhosts", "no") == "yes":
        ...
        for keyval in entries:
            if not add_key(keyval, hostkey, True):
                raise Exception(
                    "Declined host key for %s - aborting connection" % keyval
                )
    else:
        # Add host and IP entry as a single line
        if not add_key(",".join(entries), hostkey, False):
            raise Exception(
                "Declined host key for %s - aborting connection" % ",".join(entries)
            )
    ...
    def add(self, hostname, key, hash_hostname=False):
        ...
        with _lock:
            ...
            if hash_hostname or hostname.startswith("|"):
                ...
                self._index[hostname].append(lineno)
            self._lines.append("%s %s %s" % (hostname, keytype, keyval))
            if self._filename:
                self.save()
        logging.getLogger("radssh.keys").info(
            "Added new known_hosts entry for %s (%s) to %s"
            % (hostname, printable_fingerprint(key), self._filename)
        )
            ...
            for lineno, line in enumerate(f):
                ...
                try:
                    ...
                    logging.getLogger("radssh.keys").error(
                        "Skipping unloadable key line (%s:%d): %s"
        ...
        with _lock:
            if unconditional_add:
                ...
                while reply.upper() not in ("Y", "N", "A"):
                    reply = user_input(
                        "Accept new %s key with fingerprint [%s] for host %s ? (y/n/a) "
                        % (key.get_name(), fingerprint, host)
                    )
                if reply.upper() == "N":
                    return False
                if reply.upper() == "A":
...
class HostKeyEntry:
    ...
    def __init__(
        self, hostnames=None, key=None, marker=None, lineno=None, filename=None
    ):
    ...
    def from_line(cls, line, lineno=None, filename=None):
        ...
        try:
            if keytype == "ssh-rsa":
                ...
                key = paramiko.ECDSAKey(
                    data=base64.b64decode(key), validate_point=False
                )
            else:
            ...
            raise UnreadableKey(
                "Invalid known_hosts line (%s)" % e, line, lineno, filename
            )
