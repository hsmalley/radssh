...
try:
            ...
            if setup.keydata.startswith(b"ssh-rsa"):
                ...
                try:
                    self.private_key = load_pem_private_key(
                        setup.keydata,
                        password=setup.default_passphrase,
                    ...
                    passphrase = user_password(
                        "Enter passphrase for [%s]: " % setup.keyfile
                    )
                    self.private_key = load_pem_private_key(
                        setup.keydata, password=passphrase, backend=self.backend
                    )
    ...
    warnings.warn(
        Warning(
            "PyCrypto module is no longer actively maintained - consider installing cryptography"
        )
    )
    try:
        ...
        warnings.warn(
            Warning(
                "PyCrypto module does not support PKCS1_OAEP. Encrypt/Decrypt operations disabled"
            )
        )
        PKCS1_OAEP = None
