...
def star_info(cluster, logdir, cmdline, *args):
    ...
    if cluster.output_mode == "ordered":
        print(
            "Cluster output mode: ordered {} placeholders".format(
    ...
    for x in args:
        ...
        if job:
            ...
            if isinstance(res, CommandResult):
                print(
                    "%s: %s - Return Code [%s] took %0.4g seconds"
                    % (x, res.status, res.return_code, running_time)
                )
            else:
                print(
                    "%s: *** Error *** [%s] took %0.4g seconds"
                    % (x, repr(res), running_time)
                )
        else:
...
def star_result(cluster, logdir, cmdline, *args):
        ...
        if job:
            ...
            if isinstance(res, CommandResult):
                if result_file:
                    result_file.write(
                        (
                            '<<< %s: "%s" %s - Return Code [%s] took %0.4g seconds >>>\n'
                            % (
                                x,
                                res.command,
                                res.status,
    ...
    if result_file:
        result_file.close()
        cluster.console.q.put(
            (("*result", False), 'Output saved to file "%s"' % outfile.strip())
        )
...
def star_chunk(cluster, logdir, cmdline, *args):
    ...
    print(
        "Cluster chunk-factor is %s with a temporal warp of %s"
        % (cluster.chunk_size, cluster.chunk_delay)
    )
